...
const formatProducersList = (producers, limit = 50) => {
  ...
  const sortedProducers = producersList.sort((a, b) => b.votes - a.votes).slice(0, limit)
  let msg = ''
  sortedProducers.forEach(p => {
    p.share = p.votes / votesSum
    msg += `${p.rank}. ${p.owner}`.padEnd(25) + prettyNumber(p.votes).trim().padStart(20) + '\n'
  })
